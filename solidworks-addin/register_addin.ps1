...
Write-Host "✓ RegAsm.exe found" -ForegroundColor Green
...
if (-not (Test-Path $dllPath)) {
    Write-Host "Release build not found, trying Debug..." -ForegroundColor Yellow
    $dllPath = Join-Path $scriptDir "bin\Debug\$addinDll"
    Write-Host "Checking Debug path: $dllPath" -ForegroundColor Gray
}

# Check if DLL exists
Write-Host "Looking for DLL at: $dllPath" -ForegroundColor Yellow
if (-not (Test-Path $dllPath)) {
    Write-Host ""
    ...
    Write-Host "2. Build > Build Solution `(or press Ctrl+Shift+B`)"
...
Write-Host "Command: `"$regasm`" `"$dllPath`" /codebase /tlb" -ForegroundColor Gray
Write-Host ""

try {
    $output = & $regasm $dllPath /codebase /tlb 2>&1
